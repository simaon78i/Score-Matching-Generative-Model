...
class ScoreMatchingModel(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, bsz, device, noise = 1.0, x0 = None, stochastic=False, stochastic_noise_scale=1.0):
        """
        Parameters
        ----------
        ...
        num_sampling_timesteps = 500 #int(21.0 * noise)
        ...
        for idx, (scalar_sigma_start, scalar_sigma_end) in enumerate(zip(sigmas[0:-1], sigmas[1:])):
            ...
            if stochastic:
                d_sigma = scalar_sigma_end - scalar_sigma_start

                # deterministic part of the reverse SDE:
                # twice the old probability-flow ODE step
                x = pred_at_x_t.x_t + 2.0 * pred_at_x_t.pred_eps * d_sigma

                alpha = 2.0 * scalar_sigma_start * (
                            scalar_sigma_start - scalar_sigma_end)
                alpha = torch.clamp(alpha.to(device=x.device, dtype=x.dtype),
                                    min=0.0)

                fresh_noise = torch.randn_like(x)
                x = x + stochastic_noise_scale * torch.sqrt(
                    alpha) * fresh_noise
            else:
                x = self.sampler.step(scalar_sigma_start, scalar_sigma_end,
                                      pred_at_x_t)
